...
// pure presentation chrome; the canonical per-tag data lives in venue_tags. The
// `slug` matches venue_tags.facet exactly.
...
export async function fetchVenueTags(
  ...
  return data as VenueTag[];
}

// Resolves an arbitrary list of tag slugs (e.g. venues.tags) into ordered,
...
// touching the DB.
export async function resolveVenueTags(
  client: SupabaseClient,
