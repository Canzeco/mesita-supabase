// Supabase Edge Function — atlas-update-unit-data (artificial caller / agent)
//
// The UPDATE half of the ASYNC enrich pipeline. The create path
// (xxx-create-unit) inserts a MINIMAL units/places row (adea_status='generating')
...
// EXISTING row — an UPDATE keyed by venue_id, not an INSERT.
//
...
//
// Media is NOT handled here — the Enricher invokes atlas-save-place-media next
// with the media_assets get-enriched returned.
//
// Contract: verify_jwt=false; requireInternalCaller gates the service-role
// bearer. Mirrors atlas-save-unit-data / atlas-get-enriched-place gating.
//
// Local:  supabase functions serve atlas-update-unit-data
// Deploy: supabase functions deploy atlas-update-unit-data
...
Deno.serve(async (req) => {
  ...
  if (!unit) {
    return json({ ok: false, code: "unit_not_found", error: "No unit exists for this venue_id." }, 404);
  }
