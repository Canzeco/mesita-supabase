// Supabase Edge Function — business-whats-handle-message (external caller)
//
...
Deno.serve(async (req) => {
  ...
  if (!twilio.ok) {
    console.error("[business-whats-handle-message]", twilio.error);
    return new Response("Twilio not configured", { status: 500 });
  }

  const params = await parseTwilioForm(req);
  const url = webhookUrlForFunction("business-whats-handle-message");
  ...
  if (!valid) {
    console.warn("[business-whats-handle-message] invalid signature", { url });
  ...
  console.info("[business-whats-handle-message]", {
  ...
  if (!envRes.ok) {
    console.error("[business-whats-handle-message] supabase env missing");
  ...
  try {
    ...
    console.error("[business-whats-handle-message] handler error", err);
    if (staffLine) {
      await sendStaffWhatsAppReply(
        admin,
        twilio.env,
